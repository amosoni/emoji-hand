...
export default function EmojiPackLovartStyle() {
  ...
  // 暂时注释掉所有API调用
  // const { data: credits } = api.emojiPackLovartStyle.getEmojiPackCredits.useQuery(
  //   undefined,
  //   { enabled: !!session }
  // );

  // const claimBonusMutation = api.emojiPackLovartStyle.claimRegistrationBonus.useMutation({
  //   onSuccess: () => {
  //     alert('注册奖励领取成功！');
  //   },
  //   onError: (error) => {
  ...
  // const purchaseMutation = api.emojiPackLovartStyle.purchaseEmojiPackCredits.useMutation({
  //   onSuccess: () => {
  //     alert('积分购买成功！');
  //   },
  //   onError: (error) => {
  ...
  const handlePurchase = () => {
    ...
    //   package: selectedPackage as 'small' | 'medium' | 'large' | 'enterprise',
    //   quantity
    // });
  };
  ...
  return (
    <div className="bg-gradient-to-br from-purple-600 to-pink-600 rounded-lg p-6 text-white">
      <h3 className="text-xl font-bold mb-4">表情包积分系统（Lovart风格）</h3>
        ...
        <div className="bg-white/10 rounded-lg p-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
            <div>
              ...
              <span className="font-semibold text-2xl text-yellow-400">{0}</span>
            </div>
            <div>
              <span className="text-gray-300">已生成表情包：</span>
              <span className="font-semibold">{0}个</span>
            </div>
            <div>
            ...
            <div>
              <span className="text-gray-300">注册奖励：</span>
          ...
          {false && (
            <div className="mt-4">
              <button
                ...
                className="bg-yellow-500 hover:bg-yellow-600 disabled:bg-gray-500 px-4 py-2 rounded-lg font-semibold transition-colors"
              >
                {/* {claimBonusMutation.isPending ? '处理中...' : '领取注册奖励 (10积分=10张图)'} */}
                领取注册奖励 (10积分=10张图)
              </button>
            </div>
          )}
        ...
        {packages.map((pkg) => (
          ...
          >
            ...
            <div className="text-xs text-gray-300">
              单价：¥{(pkg.price / pkg.credits).toFixed(3)}/积分
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="bg-white/10 rounded-lg p-4 mb-6">
          ...
          <div className="flex flex-wrap gap-4 items-center">
            ...
            {isLoggedIn ? (
              ...
              >
                {/* {purchaseMutation.isPending ? '处理中...' : '立即购买'} */}
                立即购买
              </button>
            ) : (
      ...
      <div className="bg-white/10 rounded-lg p-4">
        ...
        <ul className="text-sm space-y-1 text-gray-300">
          <li>• 新用户注册可获得10积分奖励（10张图）</li>
          <li>• 每张表情包消耗1积分（1积分=1张图）</li>
          <li>• 积分永久有效，无过期时间</li>
          <li>• 优惠定价：¥0.4-0.6/张图</li>
          <li>• 支持生成1-10个表情包</li>
          <li>• 积分不足时无法生成表情包</li>
        </ul>
      </div>
